(ns transport.actor
  "PassengerTransportActor — the ISCO-08 8331 community transport actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one transport operation request
  ...
  The unconditional invariant: the PassengerTransportAdvisor can never
  directly commit a record the PassengerTransportGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [transport.advisor :as advisor]
            [transport.governor :as governor]
            [transport.store :as store]))

(defn build-graph
  "Build a compiled PassengerTransportActor graph. `store` implements
  `transport.store/Store`. `advisor` implements
  `transport.advisor/Advisor` (defaults to `mock-advisor`).
